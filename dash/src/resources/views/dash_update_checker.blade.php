 ...
 $(document).on('click','.dashchecker_start',function(){
  ...
  @if(!empty(config('dash.DASH_ACCESS_KEY')))
    ...
    $.ajax({
        ...
        type:'post',
        dataType:'json',
        data:{access_key:"{{ config('dash.DASH_ACCESS_KEY') }}",current_version:"{{ $dash_current_version }}", lang:"{{request("lang")}}"},
        ...
        success: function(data) {
            ...
            if(!result.status){
                ...
                $.ajax({
                    ...
                    dataType:'json',
                    type:'post',
                    data:{url:result.url,_token:'{{ csrf_token() }}',lang:"{{request("lang")}}"},
                    success: function(data){
                        $('.checkericon').removeClass('fa-spin');
                        $('.phpdashlogo').removeClass('fa-flip');
                        if(data.status == true){
