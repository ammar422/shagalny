...
//Relationship statement End
    ...
    render:function (data, type, full, meta){
      ...
      if(data?.deleted_at != null){
      buttons +=`<a href="#void" action='{{ url($DASHBOARD_PATH.'/resource/'.$resource['resourceName']) }}/restore/`+data.id+`?lang={{request('lang',app()->getLocale())}}' rowid="`+data.id+`" class="restoreRow{{ $resourceName }}"><i class="fa fa-recycle fa-1x" style="color:#c33"></i><a>`;
      }
      @endif
      return buttons;
    },
